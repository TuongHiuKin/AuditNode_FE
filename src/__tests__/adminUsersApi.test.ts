...
describe("adminUsersApi", () => {
  it("lists users with server pagination", async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: [] }); await adminUsersApi.list("alice", 25, 25);
    expect(apiClient.get).toHaveBeenCalledWith("/api/v1/admin/users", { params: { search: "alice", first: 25, max: 25 } });
  });
  it("updates identity status", async () => {
    vi.mocked(apiClient.put).mockResolvedValue({}); await adminUsersApi.setStatus("user/1", false);
    expect(apiClient.put).toHaveBeenCalledWith("/api/v1/admin/users/user%2F1/status", { enabled: false });
  });
  it("creates users and updates the SystemAdmin role using actual endpoints", async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith("/api/v1/admin/users", expect.objectContaining({ username: "alice" }));
    expect(apiClient.put).toHaveBeenCalledWith("/api/v1/admin/users/user%2F1/roles", { systemAdmin: true });
  });
});
